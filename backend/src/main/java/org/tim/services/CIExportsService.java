...
//		return getAllAvailableTranslationToMessage(translations, messages, orderedLocale);
	...
	public List<LocaleDTO> getAllSupportedLocalesInProject(String projectId) {
		Project project = projectRepository.findById(projectId).orElseThrow(() -> new EntityNotFoundException("project"));
		List<LocaleDTO> supportedLocales = new LinkedList<>();
		for (Locale localeWrapper : project.getTargetLocales()) {
			supportedLocales.add(new LocaleDTO(localeWrapper.toString(),
					localeWrapper.getDisplayLanguage(),
					localeWrapper.getDisplayCountry()));
		}
